Feature: POST Video
  ...
  Scenario: POST a single video to the API with an empty database
    ...
    When I make a POST to the "/video" method with the song details in the body
      |Song      |Artist  |PublishDate|
      |8 (circle)|Bon Iver|2016-01-01 |
    Then The response code should be 201
    And The "artist" property in the video JSON response should be "Bon Iver"
    And The "song" property in the video JSON response should be "8 (circle)"
    And The "publishDate" property in the video JSON response should be "2016-01-01T00:00:00.000Z"
  ...
  Scenario: POST a single video the the API with a populated database
    ...
    Then The response code should be 201
    And The "artist" property in the video JSON response should be "Boris"
    And The "song" property in the video JSON response should be "Message"
    And The "publishDate" property in the video JSON response should be "2008-01-01T00:00:00.000Z"
  ...
  Scenario: POST multiple songs to the API]
    ...
    Then The response code should be 201
    And The returned Video JSON should contain a song with song property set to "Buzz-In"
    And The returned Video JSON should contain a song with song property set to "Shoot!"
    And The returned Video JSON should contain a song with song property set to "Statement"
  ...
  Scenario: POST song with song property as integer
    Given I'm using the API on url "http://turing.niallbunting.com:3006/api"
    When I make a POST to the "/video" method passing the "song" property as an "integer"
      |Song       |1234      |
      |Artist     |Deerhoof  |
  ...
  Scenario: POST song with song property as integer
    Given I'm using the API on url "http://turing.niallbunting.com:3006/api"
    When I make a POST to the "/video" method passing the "artist" property as an "integer"
      |Song       |Ay That's Me|
      |Artist     |12345       |
      |PublishDate|2017-09-01  |
    Then The response code should be 400
  ...
  Scenario: POST song with song property as integer
    Given I'm using the API on url "http://turing.niallbunting.com:3006/api"
    When I make a POST to the "/video" method passing the "publishDate" property as an "integer"
      |Song       |Ay That's Me |
      |Artist     |Deerhoof     |
      |PublishDate|12456        |
    Then The response code should be 400
